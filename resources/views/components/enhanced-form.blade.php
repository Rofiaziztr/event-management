...
@props(['id', 'name', 'type' => 'text', 'label', 'placeholder' => '', 'required' => false, 'icon' => null, 'value' => '', 'error' => null])
...
<div class="form-group" x-data="{ focused: false }">
    ...
    <div class="relative">
        @if($icon)
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <svg class="w-5 h-5 text-gray-400 transition-colors duration-200" 
                     :class="focused ? 'text-yellow-500' : 'text-gray-400'"
        ...
        <input id="{{ $id }}" 
               ...
               :class="{ 
                   'border-red-300 focus:border-red-500 focus:ring-red-500': $wire && $wire.errors && $wire.errors.has('{{ $name }}'),
                   'border-green-300 focus:border-green-500 focus:ring-green-500': $wire && $wire.success && $wire.success.has('{{ $name }}')
               }"
...
@props(['type' => 'button', 'variant' => 'primary', 'size' => 'md', 'loading' => false, 'icon' => null, 'href' => null])
...
@php
    $classes = [
        ...
        'bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white focus:ring-yellow-500' => $variant === 'primary',
        'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white focus:ring-green-500' => $variant === 'success',
        'bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white focus:ring-red-500' => $variant === 'danger',
        'bg-white hover:bg-gray-50 text-gray-700 border border-gray-300 focus:ring-gray-500' => $variant === 'secondary',
    ];
...
<div x-data="{ show: true }" 
    ...
    <div class="flex items-start">
        ...
        @if($dismissible)
            <button @click="show = false" class="flex-shrink-0 ml-4 text-gray-400 hover:text-gray-600 transition-colors duration-200">
            ...
            @if($title)
                <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-yellow-50 to-white rounded-t-2xl">
                    <div class="flex items-center justify-between">
                        <h3 class="text-lg font-bold text-gray-900">{{ $title }}</h3>
                        <button @click="hide()" class="text-gray-400 hover:text-gray-600 transition-colors duration-200">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                            </svg>
                        </button>
        ...
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2 text-sm text-gray-600">
                    <span>Tampilkan:</span>
                    <select x-model="itemsPerPage" class="border border-gray-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500">
                                ...
                                @if($sortable && isset($header['key']))
                                    <svg class="w-4 h-4 text-gray-400" 
                                         :class="{
                                             ...
                                             'transform rotate-180': sortBy === '{{ $header['key'] }}' && sortDirection === 'desc'
                                         }"
                                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
