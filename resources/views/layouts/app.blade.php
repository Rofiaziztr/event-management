...
<body class="font-sans antialiased bg-gradient-to-br from-yellow-50 via-white to-yellow-100" 
      x-data="alertSystem" 
      x-init="$store.app.sidebarOpen = window.innerWidth >= 768"
      @resize.window="$store.app.sidebarOpen = window.innerWidth >= 768">
    <div class="flex h-screen">
        ...
        <aside
            ...
            <div class="p-6 border-b border-yellow-200 h-20 flex items-center justify-between shrink-0">
                ...
                <button @click="$store.app.closeSidebar()" class="md:hidden text-gray-500 hover:text-gray-700 btn-animate">
            ...
            <nav class="flex-grow p-4 space-y-3 overflow-y-auto">
                ...
                <a href="{{ route($dashboardRoute) }}"
                    ...
                    <svg class="w-5 h-5 md:w-5 md:h-5 lg:w-6 lg:h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor">
                ...
                @if (Auth::user()->role == 'admin')
                    ...
                    <a href="{{ route('admin.users.index') }}"
                        ...
                        <svg class="w-4 h-4 md:w-5 md:h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round"
        ...
        {{-- Mobile overlay --}}
        <div x-show="$store.app.sidebarOpen && window.innerWidth < 768" 
        ...
        <div class="flex-1 flex flex-col overflow-y-auto">
            ...
            <div class="bg-white border-b border-yellow-200 sticky top-0 z-10 shadow-sm">
                <div class="max-w-full md:max-w-7xl lg:max-w-[90%] xl:max-w-[95%] 2xl:max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                    <div class="flex-1">
                        @if (isset($header))
                            {{ $header }}
                        @else
                    ...
                    <div class="md:hidden" x-data="{open: false}">
                        ...
                        <div x-show="open"
                             ...
                             class="absolute right-4 mt-2 w-56 origin-top-right rounded-xl bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                            <div class="p-2">
                                <a href="{{ route('profile.edit') }}" 
                                   @click="open = false"
                ...
                <div class="fixed top-4 right-4 z-50 space-y-3" x-data="{}" style="max-width: 340px;">
                    <template x-for="alert in $store.app.alerts" :key="alert.id">
                        <div x-show="alert.show"
                             ...
                             :class="{
                                'bg-gradient-to-r from-green-500/20 to-green-100/30 border-green-300': alert.type === 'success',
                                'bg-gradient-to-r from-red-500/20 to-red-100/30 border-red-300': alert.type === 'error',
                                'bg-gradient-to-r from-yellow-500/20 to-yellow-100/30 border-yellow-300': alert.type === 'warning',
                                'bg-gradient-to-r from-blue-500/20 to-blue-100/30 border-blue-300': alert.type === 'info'
                             }">
                            ...
                            <div class="flex items-start">
                                ...
                                <div class="p-4 flex-shrink-0 flex justify-center items-center w-14 h-full"
                                     :class="{
                                        'bg-gradient-to-br from-green-500/30 to-green-600/30': alert.type === 'success',
                                        'bg-gradient-to-br from-red-500/30 to-red-600/30': alert.type === 'error',
                                        'bg-gradient-to-br from-yellow-500/30 to-yellow-600/30': alert.type === 'warning',
                                        'bg-gradient-to-br from-blue-500/30 to-blue-600/30': alert.type === 'info'
                                     }">
                                    <span x-show="alert.type === 'success'" class="text-2xl text-green-600 animate-bounce" x-text="alert.icon"></span>
                                    <span x-show="alert.type === 'error'" class="text-2xl text-red-600 animate-pulse" x-text="alert.icon"></span>
                                    <span x-show="alert.type === 'warning'" class="text-2xl text-yellow-600 animate-pulse" x-text="alert.icon"></span>
                                    <span x-show="alert.type === 'info'" class="text-2xl text-blue-600 animate-pulse" x-text="alert.icon"></span>
                                </div>
