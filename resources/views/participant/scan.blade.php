        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function onScanSuccess(decodedText, decodedResult) {
                    ...
                    if (!latVal || !lngVal) {
                        const showFailedPresenceWithName = function(eventTitle) {
                            if (resultsElement) {
                            ...
                            <div class="text-center">
                                <div class="p-6 bg-gradient-to-br from-red-50 to-red-100 rounded-xl border-2 border-red-200 inline-block">
                                    ...
                                    <h4 class="text-lg font-bold text-red-800 mb-2">Gagal Presensi</h4>
                                      <p class="text-sm text-red-600">Presensi untuk event '${eventTitle}' gagal karena izin lokasi belum diberikan. Silakan aktifkan izin lokasi, muat ulang halaman, lalu scan QR kembali untuk presensi.</p>
                                </div>
                            </div>
                        `;
                        }

                        // Try to get event name from server to display a nicer message
                        fetch("/scan/event-name?code=" + encodeURIComponent(decodedText), { credentials: 'same-origin' })
                            .then((r) => r.ok ? r.json() : null)
                            .then((data) => {
                                const title = data && data.event && data.event.title ? data.event.title : decodedText;
                                showFailedPresenceWithName(title);
                            }).catch(() => {
                                // fallback to using the scanned code if fetch fails
                                showFailedPresenceWithName(decodedText);
                            });
