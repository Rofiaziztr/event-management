    ...
    <x-slot name="header">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
            ...
            <a href="{{ route('scan.index') }}" 
               class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-yellow-500 to-yellow-600 border border-transparent rounded-xl font-semibold text-white text-sm hover:from-yellow-600 hover:to-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-xl border border-yellow-200 card-hover">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900">{{ ($events->total() ?? 0) - count($attendedEventIds ?? []) }}</p>
                        </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-xl border border-yellow-200 card-hover">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900">{{ $events->where('status', 'Berlangsung')->count() ?? 0 }}</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col md:flex-row gap-4">
                            <div class="flex-1">
                                ...
                                <x-date-picker-all-past name="start_date" id="start_date" :value="request('start_date')" placeholder="Pilih Tanggal" />
                            </div>

                            <div class="flex-1">
                                <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">Sampai Tanggal</label>
                                <x-date-picker-all-past name="end_date" id="end_date" :value="request('end_date')" placeholder="Pilih Tanggal" />
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
                @forelse ($events as $event)
                    <a href="{{ route('participant.events.show', $event) }}" class="block">
                        <div class="bg-white rounded-2xl shadow-xl border border-yellow-200 overflow-hidden flex flex-col relative group transition-all duration-300 card-hover">
                            <div class="p-6 flex-grow z-10">
                                <div class="flex justify-between items-start mb-2">
                                    <h3 class="font-semibold text-lg text-gray-900 group-hover:text-yellow-600 line-clamp-2">
