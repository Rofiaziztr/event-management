            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-4 sm:px-0">
                @forelse ($events as $event)
                    <a href="{{ route('participant.events.show', $event) }}" class="block">
                        <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden flex flex-col relative group transition-all duration-300 hover:shadow-xl">
                            <div class="p-6 flex-grow z-10">
                                {{-- Card Header --}}
                                <div class="flex justify-between items-start mb-5">
                                    <h3 class="font-semibold text-lg text-gray-900 leading-tight hover:text-indigo-600 transition-colors line-clamp-2">
                                        {{ $event->title }}
                                    </h3>
                                    ...
                                    @if ($dynamicStatus == 'Terjadwal')
                                        <x-bladewind::tag label="{{ $dynamicStatus }}" color="blue" size="small" />
                                    @elseif($dynamicStatus == 'Berlangsung')
                                        <x-bladewind::tag label="{{ $dynamicStatus }}" color="green" size="small" />
                                    @elseif($dynamicStatus == 'Selesai')
                                        <x-bladewind::tag label="{{ $dynamicStatus }}" color="gray" size="small" />
                                    @else
                                        <x-bladewind::tag label="{{ $dynamicStatus }}" color="red" size="small" />
                                    @endif
                                </div>

                                {{-- Card Body with Details and Icons --}}
                                <div class="space-y-4 mt-4 text-sm text-gray-600">
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-gray-700">{{ $event->start_time->format('d M Y, H:i') }} WIB</span>
                                    </div>
                                    <div class="flex items-center">
                                        <svg class="w-5 h-5 mr-3 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                        <span class="text-gray-700">{{ $event->countdown_status }}</span>
                                    </div>
                                    <div class="flex items-start">
                                        ...
                                        <span class="text-gray-700 line-clamp-1">{{ $event->location }}</span>
                                    </div>
                                    <div class="flex items-center">
                                        <svg class="w-5 h-5 mr-3 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                        </svg>
                                        <span class="text-gray-700">{{ $event->participants->count() }} peserta</span>
                                    </div>
                                </div>
