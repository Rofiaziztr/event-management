    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            <div class="flex items-center space-x-4">
                {{-- Avatar --}}
                <div
                    class="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center text-white shadow-lg">
                ...
                <div>
                    ...
                    @if ($user->position)
                        <span
                            class="mt-1 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                            {{ $user->position }}
                        </span>
                    @endif
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-xl border border-yellow-200">
                        ...
                        <div class="p-6 space-y-4 text-sm">
                            ...
                            <div>
                                ...
                                <p class="font-medium text-gray-800">{{ $user->created_at->isoFormat('D MMMM YYYY') }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-xl border border-yellow-200">
                        ...
                        <div class="p-6 space-y-3">
                            <a href="{{ route('admin.users.edit', $user) }}"
                                class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 border border-transparent rounded-xl font-semibold text-sm text-white hover:from-indigo-600 hover:to-indigo-700 transition">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <a href="{{ route('admin.users.export-participant-detail', $user) }}"
                                class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-green-500 to-green-600 border border-transparent rounded-xl font-semibold text-sm text-white hover:from-green-600 hover:to-green-700 transition">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                Export Detail Peserta
                            </a>
                            <form action="{{ route('admin.users.destroy', $user) }}" method="POST"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-red-500 to-red-600 border border-transparent rounded-xl font-semibold text-sm text-white hover:from-red-600 hover:to-red-700 transition">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div
                            class="bg-white rounded-2xl p-6 shadow-xl border border-yellow-200 flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-500">Total Event Diikuti</p>
                                <p class="text-3xl font-bold text-gray-900">{{ $user->participatedEvents->count() }}
                                </p>
                            </div>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div class="flex-1 min-w-0">
                                            <a href="{{ route('admin.events.show', $event) }}" class="block group">
                                                <p
                                                    class="font-medium text-gray-900 group-hover:text-indigo-600 truncate transition-colors">
                                                    {{ $event->title }}</p>
                                                <p class="text-sm text-gray-500 mt-1">
                                                    {{ $event->start_time->isoFormat('dddd, D MMMM YYYY') }}</p>
                                            </a>
                                        </div>
                                        <div class="ml-4">
                                            @php
                                            ...
                                            @else
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">Absen</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
