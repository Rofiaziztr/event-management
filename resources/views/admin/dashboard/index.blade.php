        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <form method="GET" class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div>
                            ...
                            <select name="chart_type" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        <div>
                            ...
                            <select name="group_by" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-200">
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('dashboard', ['start_date' => now()->startOfDay()->format('Y-m-d'), 'end_date' => now()->endOfDay()->format('Y-m-d')]) }}" 
                               class="px-3 py-1 text-xs bg-blue-100 text-blue-700 rounded-full hover:bg-blue-200 transition-colors">
                                Hari Ini
                            </a>
                            <a href="{{ route('dashboard', ['start_date' => now()->startOfWeek()->format('Y-m-d'), 'end_date' => now()->endOfWeek()->format('Y-m-d')]) }}" 
                               class="px-3 py-1 text-xs bg-green-100 text-green-700 rounded-full hover:bg-green-200 transition-colors">
                                Minggu Ini
                            </a>
                            <a href="{{ route('dashboard', ['start_date' => now()->startOfMonth()->format('Y-m-d'), 'end_date' => now()->endOfMonth()->format('Y-m-d')]) }}" 
                ...
                <div class="bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl p-6 text-white shadow-lg">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-bold">{{ number_format($averageAttendance, 1) }}%</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-gradient-to-r from-emerald-500 to-emerald-600 px-6 py-4">
                    <div class="flex items-center justify-between">
                            ...
                            <h3 class="text-lg font-semibold text-white">
                                @php
                                    ...
                                    $title = match($groupBy) {
                                        ...
                                        default => 'Top 5 Event Berdasarkan Kehadiran'
                                    };
                                @endphp
                                {{ $title }}
                            </h3>
                        ...
                        <div class="flex items-center space-x-2 text-emerald-100 text-sm">
                            ...
                            <span>{{ ucfirst($chartType) }} Chart</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                    ...
                    @if($eventAttendance->isEmpty())
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-500 text-sm">Tidak ada data untuk periode yang dipilih</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6">
                        {{-- This would show recent events, registrations, etc. --}}
                        <div class="space-y-3">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-semibold">{{ $monthlyEvents ?? 0 }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-500">Kehadiran Bulan Ini:</span>
                            <span class="font-semibold">{{ $monthlyAttendances ?? 0 }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-500">Event Aktif:</span>
                            <span class="font-semibold">{{ $activeEvents ?? 0 }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-500">Total Dokumen:</span>
                            <span class="font-semibold">{{ $totalDocuments ?? 0 }}</span>
                        </div>
                    </div>
                </div>
            </div>
