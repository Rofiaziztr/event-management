<div class="flex items-center space-x-2">
    ...
    @if(!$p->attendances->where('event_id', $event->id)->count())
        <form action="{{ route('admin.events.participants.manual', [$event, $p]) }}" method="POST" class="inline">
            @csrf
            <button type="submit" class="inline-flex items-center px-2 py-1 md:px-3 md:py-1.5 lg:px-3 lg:py-1.5 bg-green-100 text-green-700 text-xs md:text-sm lg:text-sm font-medium rounded hover:bg-green-200 transition-colors">
                <svg class="w-3 h-3 md:w-4 md:h-4 lg:w-4 lg:h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
