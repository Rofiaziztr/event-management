            ...
            <div class="p-6 border-b border-gray-100">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h3 class="text-lg md:text-xl font-bold text-gray-900">Lampiran Acara</h3>
                            <p class="text-xs md:text-sm text-gray-500 mt-1">Dokumen, foto, atau video pendukung acara.</p>
                        </div>
                    </div>
            ...
            <div class="divide-y divide-gray-100">
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex items-start space-x-4 flex-1 min-w-0">
                                ...
                                <div class="flex-shrink-0">
                                    @php
                                        ...
                                        $iconClass = 'w-8 h-8 rounded-lg flex items-center justify-center text-white text-xs font-semibold shadow-sm';
                                        $fileIcons = [
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-xs sm:text-sm font-semibold text-gray-900 truncate" title="{{ $document->title }}">
                                        {{ $document->title }}
                                    </h4>
                                    <div class="flex flex-wrap items-center text-xs text-gray-500 mt-1 gap-x-2">
                                        ...
                                        @if (Storage::exists($document->file_path))
                                            <span class="flex items-center">
                                                ...
                                                <span>{{ strtoupper($extension) }} &middot; {{ number_format(Storage::size($document->file_path) / 1024, 1) }} KB</span>
                                            </span>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center space-x-1 ml-2">
                                ...
                                <form action="{{ route('admin.documents.destroy', $document) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors duration-150 border border-transparent hover:border-red-200 shadow-sm"
                                            title="Hapus">
                ...
                @empty
                    <div class="p-6 text-center">
                        ...
                        <p class="text-gray-500 text-xs mb-1.5">Belum ada lampiran yang diunggah</p>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-2xl shadow-xl border border-yellow-200 overflow-hidden">
            <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 px-6 py-4 rounded-t-2xl">
                <div class="flex items-center">
                    <svg class="w-4 h-4 text-white mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                    <h3 class="text-sm font-semibold text-white">Unggah Lampiran Baru</h3>
                </div>
            </div>
            <form action="{{ route('admin.events.documents.store', $event) }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="document_title" class="block text-xs font-medium text-gray-700 mb-1">
                    ...
                    <div class="flex justify-end pt-3">
                        <x-bladewind::button can_submit="true" color="indigo" icon="cloud-upload" has_spinner="true" class="text-sm shadow-md hover:shadow-lg transition-shadow duration-200">
                            Unggah File
                        </x-bladewind::button>
                    </div>
                </div>
        ...
        <div class="bg-blue-50 rounded-xl border border-blue-200 p-4">
            <div class="flex items-start">
                <div class="flex-shrink-0">
                    <svg class="h-4 w-4 text-blue-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/></svg>
                </div>
                <div class="ml-2">
                    <h3 class="text-xs font-medium text-blue-800">Panduan Upload</h3>
