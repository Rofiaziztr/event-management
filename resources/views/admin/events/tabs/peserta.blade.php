<div class="space-y-8">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-yellow-200">
        <div class="p-4 md:p-6 border-b border-gray-100">
            <h3 class="text-lg md:text-xl font-bold text-gray-900">Undang Peserta</h3>
            <p class="text-xs md:text-sm text-gray-500 mt-1">Undang peserta internal yang sudah terdaftar atau tambahkan peserta eksternal baru.</p>
        </div>
        <div class="border-b border-gray-100">
            <nav class="flex px-6">
                <button type="button" data-tab="internal" class="tab-btn w-1/2 py-3 md:py-4 px-1 text-center border-b-2 font-medium text-xs md:text-sm border-yellow-500 text-yellow-600 tab-transition hover:bg-yellow-50">Internal</button>
                <button type="button" data-tab="external" class="tab-btn w-1/2 py-3 md:py-4 px-1 text-center border-b-2 font-medium text-xs md:text-sm border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 tab-transition hover:bg-gray-50">Eksternal</button>
            </nav>
        </div>
    ...
    {{-- Daftar Peserta --}}
    <div class="bg-white rounded-2xl shadow-xl border border-yellow-200">
        <div class="p-4 md:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            <div>
                <h3 class="text-lg md:text-xl font-bold text-gray-900">Daftar Peserta <span class="text-yellow-600 bg-yellow-100 px-2 py-1 rounded-full text-xs font-medium">{{ $event->participants->count() }}</span></h3>
                <div class="mt-1 flex items-center">
                    <span class="text-xs text-gray-500 mr-2">Status Event:</span>
                    <span class="text-xs font-medium px-2 py-0.5 rounded-full
                        @if($event->status === 'Berlangsung') bg-green-100 text-green-800
                        @elseif($event->status === 'Terjadwal') bg-blue-100 text-blue-800
            ...
            <button id="bulk-attendance-btn" 
                class="w-full sm:w-auto px-3 py-2 md:px-4 md:py-2 
                       {{ $event->status === 'Selesai' ? 'bg-orange-500' : 'bg-gradient-to-r from-yellow-500 to-yellow-600' }}
                       text-white rounded-lg text-xs md:text-sm font-medium shadow-sm hover:shadow-md transition-shadow duration-200 disabled:opacity-50" 
                disabled>
...
document.getElementById('bulk-attendance-btn')?.addEventListener('click', async () => {
    ...
    if (eventStatus === 'Selesai') {
        confirmMessage = `Event ini sudah selesai. Anda akan menambahkan kehadiran secara retroaktif untuk ${ids.length} peserta. Lanjutkan?`;
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (eventStatus === 'Selesai' || data.event_status === 'Selesai') {
                successMessage = `${data.success} (Kehadiran ditambahkan secara retroaktif karena event sudah selesai)`;
                // Gunakan tipe notifikasi warning untuk event yang sudah selesai
                notificationType = 'warning';
                // Tambahkan notifikasi khusus untuk admin
                showNotification('warning', 'PERHATIAN: Anda telah menambahkan kehadiran secara retroaktif karena event ini sudah selesai. Ini adalah fitur khusus admin.');
            }
            ...
            if (data.error === 'Event tidak sedang berlangsung.') {
                ...
                if (eventStatus === 'Terjadwal') {
                    showNotification('error', 'Event belum dimulai. Kehadiran hanya dapat dicatat saat event sedang berlangsung.');
                } else {
...
function showNotification(type, message) {
    ...
    if (window.Alpine && Alpine.store('app')) {
        ...
        let icon = '';
        if (type === 'error') {
            icon = '<svg class="w-5 h-5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';
        } else if (type === 'warning') {
...
function initializeTabs() {
    ...
    const activeTab = document.querySelector('.tab-btn.border-yellow-500') || document.querySelector('.tab-btn[data-tab="internal"]');
    if (activeTab) {
