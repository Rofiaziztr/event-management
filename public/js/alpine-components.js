    ...
    Alpine.store("app", {
        ...
        addAlert(type, message, duration = 5000, options = {}) {
            ...
            if (options.sound !== false && window.Audio) {
                try {
                    // You can add actual sound files in your public directory
                    // const audio = new Audio(`/sounds/${sounds[type] || 'notification'}.mp3`);
                    // audio.volume = 0.5;
                    // audio.play().catch(e => console.log('Sound could not play automatically'));
                } catch (e) {
                    // Fail silently
                }
            }
            ...
            // Vibrate device if browser supports it (mobile devices)
            if (options.vibrate !== false && navigator.vibrate) {
                try {
