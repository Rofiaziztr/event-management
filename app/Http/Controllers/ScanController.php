...
class ScanController extends Controller
{
    ...
    /**
     * Return event details (title) by code as JSON for client-side display.
     */
    public function getEventName(Request $request)
    {
        $code = $request->query('code');
        if (!$code) {
            return response()->json(['success' => false, 'message' => 'Kode event diperlukan.'], 400);
        }
        ...
        return response()->json(['success' => true, 'event' => ['id' => $event->id, 'title' => $event->title]]);
    }
}
